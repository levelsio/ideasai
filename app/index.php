	...
	// <front page>
		if(empty($_GET['action']) && php_sapi_name()!='cli') {
						...
						$('body').on('mouseup touchend',function(e) {
							...
							/* <check if dragged to left/right> */
								...
								if(dragRelativeX<-100) {
									...
									/* return to center */
									$('.center-idea-container .table').addClass('transition');
									$('.center-idea-container .table').css('transform','translate3d(0px,0px,0px) rotate(0deg)');
									setTimeout(function() {
										$('.center-idea-container .table').removeClass('transition');
									},125);
								}
					...
					$('.action-upvote').bind('click',function() {
						...
						activeAjax=$.ajax({
							...
							if(soloIdea) {
								...
								$('.tr#id_'+reply.callback_id).find('.action-upvote').data('id',reply.new_idea.id).css('opacity',1);
								$('.tr#id_'+reply.callback_id).find('.action-downvote').data('id',reply.new_idea.id).css('opacity',1);
								$('.tr#id_'+reply.callback_id).find('.votes').data('votes',reply.new_idea.votes);
								$('.tr#id_'+reply.callback_id).find('.votes').text(number_format(reply.new_idea.votes));
			...
			<?if(!$_COOKIE['ideasai_subscribed']){?>
				<div class="banner-subscribe" style="position:fixed;left:0;width:100%;bottom:0;text-align:center;line-height:1.8;background:#000;color:#fff;padding:14px;box-shadow:0 -1px -2px 0 rgba(0,0,0,.1);margin:0 auto;z-index:100;border-top:1px solid #eee;">
			...
			*/?>
			...
			<p clas="text" style="text-align:center;line-height:1.8;">
				<strong>
					Ideas on this page are 100% generated by <a href="https://openai.com/">OpenAI</a>'s GPT-3 and the model is trained by you. <a href="#readme">How it works</a>
				</strong>
			</p>
...
If you don't want to get these weekly ideas anymore, <a href=\"https://ideasai.net/?action=stop_receiving&email=".urlencode($email['email'])."&hash=".$hash."\">click here</a>";
	...
	// <generating ideas>
			...
			// $query=$gpt3ideasDb->prepare("SELECT * FROM gpt3ideas WHERE human_seeded=1 AND launch_hn IS NOT 1 ORDER BY random() LIMIT 125");
			...
			// // <get bad idea>
			// 	$query=$gpt3ideasDb->prepare("SELECT * FROM gpt3ideas WHERE votes<-1 ORDER BY random() LIMIT 12");
			// 	$query->execute();
			...
			?><div class="table" style="<?if($soloIdea){?>transform:rotate(0deg);<?}?>margin:14px auto;"><?
				...
				?><div id="id_<?=$idea['id']?>" class="tr container"><?
					?><div class="td td_idea">
						...
						<span class="votes" data-votes="<?=$idea['votes']?>"><?
