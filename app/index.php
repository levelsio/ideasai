<?
	...
	$emailSponsorHTML="<strong>Sponsor: <a href=\"https://remoteok.io/?ref=ideasai\">You don't need to go back to the office! Find a remote job on Remote OK and work from anywhere</a></strong><br/><br/>\n\n";
		...
		if(empty($_GET['action']) && php_sapi_name()!='cli') {
			...
			// <tag page>
				if($_GET['tag']) {
					...
					// $pageTitle=number_format(count($topIdeas)).' '.capitalize($_GET['tag']).' Startup Ideas generated by GPT-3';
					// $pageTitle=number_format(count($topIdeas)).' '.capitalize($_GET['tag']).' Startup Ideas';
					$pageTitle=capitalize($_GET['tag']).' Startup Ideas';
			...
			// $query=$gpt3votesDb->prepare("SELECT COUNT(*) FROM gpt3votes");
			// $query->execute();
			...
			if(!$pageTitle) {
				// $pageTitle='Ideas AI: GPT-3-powered startup idea generator';
				$pageTitle='Startup Ideas powered by OpenAI';
			}
			...
			<meta name="twitter:description" content="Ideas AI is an A.I. that generates startup ideas using GPT-3 by OpenAI" />
			...
			<meta property="og:description" content="Ideas AI is an A.I. that generates startup ideas using GPT-3 by OpenAI" />
					...
					$('.action-upvote').bind('click',function() {
						...
						activeAjax=$.ajax({
							...
							if(soloIdea) {
								...
								// $('.tr#id_'+reply.callback_id).find('.time_ago').text('Generated by OpenAI, '+reply.new_idea.time_ago+' ago');
			...
			<p style="margin-top:-28px;">
				<h2 style="font-size:16px;margin:0 auto;font-style:normal;">
					Ideas on this page are 100% generated by OpenAI's GPT-3, an artifically intelligent deep learning model, without human involvement, and trained by you and <?=number_format($uniqueVoteCount)?>+ other people who liked or disliked ideas. <a href="/#readme">How it works</a>
				</h2>
			</p>
			...
			<?if(!$_COOKIE['ideasai_subscribed']){?>
				<div class="banner-subscribe" style="position:fixed;left:0;width:100%;bottom:0;text-align:center;line-height:1.8;background:#000;color:#fff;padding:14px;box-shadow:0 -1px -2px 0 rgba(0,0,0,.1);margin:0 auto;z-index:100;">
					<strong style="width:calc(100% - 28px);display:inline">
						Join <?=number_format($emailCount)?> people who get the best new startup ideas by OpenAI in their email weekly
					</strong>
			...
			*/?>
			...
			<?if($frontpage){?>
				...
				<p clas="text" style="text-align:left;line-height:1.8;">
					All ideas are 100% generated by OpenAI's GPT-3. With no human involvement whatsoever. The database was seeded with ~100 startup ideas. From there GPT-3 started generating more. The generated ideas are then liked/disliked by you and <?=number_format($uniqueVoteCount)?>+ other people. And that data is fed back into GPT-3. That means with time the ideas GPT-3 generates get better and better.
				</p>
				<p clas="text" style="text-align:left;line-height:1.8;">
	...
	// <subscribe email>
			...
			// <send confirm email>
				...
				$sendGridMail->setFrom('no-reply@ideasai.net','Ideas AI');
				$sendGridMail->setSubject("Confirm your email on Ideas AI");
				$sendGridMail->addTo($_GET['email'],$_GET['email']);
				$sendGridMail->addContent("text/plain","Hi,

You (or someone else) entered your email to subscribe to Ideas AI.
...
");
	...
	// <send out emails>
			...
			foreach($emailsToSend as $email) {
				...
				$sendGridMail = new \SendGrid\Mail\Mail(); 
				$sendGridMail->setFrom('no-reply@ideasai.net','Ideas AI');
				$sendGridMail->setSubject("Your weekly ideas from 💡 Ideas AI");
				$sendGridMail->addTo($email['email'],$email['email']);
				$message="<strong>Here's your weekly startup ideas generated by OpenAI from <a href=\"https://ideasai.net\">💡 Ideas AI</a>:</strong><br/><br/>
".$emailSponsorHTML."
".$weeklyTopIdeasList."
<br/>
			...
			?><div class="table" style="<?if($soloIdea){?>transform:rotate(0deg);<?}?>margin:14px auto;"><?
				...
				?><div id="id_<?=$idea['id']?>" class="tr container"><?
					?><div class="td td_idea">
						...
						?></h3><?
						// echo '<br/><span class="time_ago">Generated by OpenAI, '.timeAgoLong($idea['epoch_created']).' ago. </span>';
